package com.seangogo.blogs.config.shiro.session;


import com.seangogo.blogs.config.shiro.CustomShiroSessionDAO;
import com.seangogo.blogs.domain.Admin;
import com.seangogo.blogs.utils.LoggerUtils;
...
public class CustomSessionManager {
	...
	public  List<Admin> getAllUser() {
		...
		for (Session session : sessions) {
			Admin admin = getSessionBo(session);
			if(null != admin){
				list.add(admin);
			}
		}
		...
		for (Session session : sessions) {
			...
			if(null != obj && obj instanceof SimplePrincipalCollection){
				...
				if(null != obj && obj instanceof Admin){
					...
					//比较用户ID，符合即加入集合
					if(null != admin && idset.contains(admin.id())){
						list.add(spc);
					}
				}
			}
	 ...
	 * @return
	 */
	public Admin getSession(String sessionId) {
		return getSessionBo(shiroSessionRepository.getSession(sessionId));
	}

	private Admin getSessionBo(Session session){
		//获取session登录信息。
		Object obj = session.getAttribute(DefaultSubjectContext.PRINCIPALS_SESSION_KEY);
		if(null == obj){
			return null;
		...
		if(obj instanceof SimplePrincipalCollection){
			...
			/*if(null != obj && obj instanceof Admin){
				//存储session + user 综合信息
				UserOnlineBo userBo = new UserOnlineBo((Admin)obj);
				//最后一次和系统交互的时间
	...
	public void forbidUserById(Long id, Long status) {
		/*//获取所有在线用户
		for(Admin admin : getAllUser()){
			String userId = admin.id();
			//匹配用户ID
			if(userId.equals(id)){
				//获取用户Session
				Session session = shiroSessionRepository.getSession(admin.getSessionId());
				//标记用户Session
				SessionStatus sessionStatus = (SessionStatus) session.getAttribute(SESSION_STATUS);
